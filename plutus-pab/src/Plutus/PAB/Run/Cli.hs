...
runConfigCommand _ _ _ ConfigCommandArgs{ccaTrace, ccaPABConfig = Config {nodeServerConfig},ccaAvailability} StartMockNode =
    ...
    do
        ...
        result <- App.runApp ccaStorageBackend (toPABMsg ccaTrace) contractHandler config
          $ do
              ...
              (mvar, _) <- PABServer.startServer' extraServer extraServerProxy pabWebserverConfig (Left walletClientEnv) ccaAvailability
...
runConfigCommand extraServer extraServerProxy contractHandler c@ConfigCommandArgs{ccaAvailability} (ForkCommands commands) =
    void $ do
